...
abstract class AbstractEntity<T : Entity<T>> : AbstractMutableSpatial(), Entity<T> {
     ...
     */
    protected open fun findContacts(ents: Sequence<T>): Collection<T> {
        contacts.clear()
        for (ent in ents) {
    ...
    override fun handleCollisionsAndGetContacts(ents: Sequence<T>): Collection<T> {
        val potentialContacts = ents.asSequence().filter {
            this !== it && this.boundingBox.isTouching(it.boundingBox)
        }
        val contacts = findContacts(potentialContacts)
